	...
	for filename in os.listdir(datadir):
		...
		for name, v in dict.iteritems(data):
			for date, text in dict.iteritems(v):
				...
				exclude = exclude | {"`", "--", "'", "..."}
				s2 = [[word for word in sentence if word not in exclude] for sentence in words]
				...
				if election_name != "Hillary Clinton republican 2008":
					megadict[election_name][date] = s2
	newf = codecs.open('full_parsed_sentences.json' , 'w', encoding='utf-8')
